...
def prepare_dataloader(df , batch_size, label_task , epoch_switch , pin_memory=True, num_workers=2 , check = "train" , accum = False ): 
    ...
    if accum:
        batch_size = 1
        dataset = TextAudioDataset(df , batch_size , max_len=int(70*2.5) , feature_col1="audio_path" , feature_col2="text" , label_col=label_task , timings="timings" , speaker="speaker")
    else:
        dataset = TextAudioDataset(df , batch_size , max_len=int(70*2.5) , feature_col1="audio_path" , feature_col2="text" , label_col=label_task , timings="timings" , speaker="speaker")
...
def runModel( accelerator, df_train , df_val, df_test  ,param_dict , model_param):
    ...
    model = train_ta_network(model , [df_train_no_accum , df_train_accum], df_val, criterion , lr, epoch ,  weight_decay,T_max, Metric , patience , clip , epoch_switch , checkpoint )
...
def main():
    ...
    param_dict = {'epoch': 6, 'patience': 10, 'lr': 5e-06, 'clip': 5, 'batch_size': 1, 'weight_decay': 5e-05, 'model': 'MAE_encoder', 'T_max': 3, 'seed': config.seed, 'label_task': 'emotion', 'mask': False, 'loss': 'NewCrossEntropy', 'beta': 1, 'epoch_switch': 2}
