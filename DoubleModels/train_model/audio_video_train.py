...
def get_statistics(input , label , model , criterion , Metric , check="train" , epoch = None):
    ...
    output = model( audio_input_ids.to(device) ,
...
def get_statistics_big_batch(input , label , model , criterion , Metric , check="train" , epoch = None):
    ...
    output = checkpoint(model , audio_input_ids.to(device) ,
...
def train_av_network(model , train_dataloader, val_dataloader, criterion,learning_rate, epochs , weight_decay , T_max ,Metric, patience , clip , epoch_switch , checkpoint = None):
...
def evaluate_av(model , test_dataloader , Metric):
