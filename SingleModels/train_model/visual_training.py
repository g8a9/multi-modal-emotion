...
def get_statistics(input , label , model , criterion , Metric , check="train" , epoch = None):
    ...
    video_input_ids = input["visual_embeds"]
    video_attention_mask = input["attention_mask"]  
    del input
    
    output = model(video_input_ids.to(device) , video_attention_mask.to(device) , check)  
...
def get_statistics_big_batch(input , label , model , criterion , Metric , check="train" , epoch = None):
    ...
    output = checkpoint(model , video_input_ids.to(device) , video_attention_mask.to(device) , check , use_reentrant=False)  
...
def grad_accum(epoch , train_dataloader , val_dataloader , model , criterion , optimizer , scheduler, clip , patience , Metric , prev_val_loss , prev_f1 , total_loss_train , iters , log_val , path):
    ...
    fn = get_statistics_big_batch if batch_size > 64 else get_statistics
...
def train_video_network(model , train_dataloader, val_dataloader, criterion,learning_rate, epochs , weight_decay , T_max ,Metric, patience , clip , epoch_switch , checkpoint = None):
...
def evaluate_video(model , test_dataloader , Metric):
