...
def get_statistics(input,label,model,criterion, Metric , check = "train",epoch = None):
    ...
    output = model(input_values , check) # our output should be of size : (batch_size x num_classes) so 4x7 -> argmax goes to 4x1
    ...
    for i in range( (len(train_dataloader) // log_val) + 1):
        for j in range(log_val):
            try:
                ...
                break
        prev_val_loss = run_validation(epoch  , val_dataloader , model , criterion , optimizer , scheduler  , Metric , prev_val_loss , total_loss_train/iters , batch_idx  , log_val , path )
...
def run_validation(epoch  , val_dataloader , model , criterion , optimizer , scheduler , Metric , prev_val_loss , curr_loss , step , log_val , path):
    global PATIENCE_ITER
...
def one_epoch(epoch , train_dataloader , val_dataloader , model , criterion , optimizer , scheduler, clip , epoch_switch , patience , Metric , prev_val_loss):
    ...
    log_val = 2400
    path = "/home/prsood/projects/def-whkchun/prsood/Audio_Train"
...
def train_audio_network(model , train_dataloader, val_dataloader, criterion,learning_rate, epochs , weight_decay , T_max ,Metric, patience , clip , epoch_switch , checkpoint = None):
