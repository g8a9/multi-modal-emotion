...
import os
from .train_model.audio_training import train_audio_network , evaluate
...
def prepare_dataloader(df , batch_size, label_task , epoch_switch , pin_memory=True, num_workers=8 , check = "train" ): # num_W = 8 kills it 
    ...
    if check == "train":
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, pin_memory=pin_memory, 
                num_workers=num_workers ,drop_last=False, shuffle=False, sampler = sampler, collate_fn=collate_batch)
    else:
        dataloader = DataLoader(dataset, batch_size=batch_size, pin_memory=pin_memory, 
                num_workers=num_workers ,drop_last=False, shuffle=False, collate_fn=collate_batch)
