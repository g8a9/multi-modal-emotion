...
class NewCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, train_class_weights , val_class_weights , epoch_switch = 2):
        super().__init__()
        self.train_class_weights = train_class_weights
        self.val_class_weights = val_class_weights
        self.epoch_switch = epoch_switch
        self.TrainweightedCEL = torch.nn.CrossEntropyLoss(weight=train_class_weights)
        self.ValweightedCEL = torch.nn.CrossEntropyLoss(weight=val_class_weights)
        self.normalCEL = torch.nn.CrossEntropyLoss()
        self.iter1 = 1
        self.iter2 = 1
        self.iter3 = 1

    def forward(self, logits, target , epoch , check = "train" ):
        if check == "train":
            if epoch % self.epoch_switch == 0:
                ...
                if self.iter2 == 1 or self.iter3 == 1200:
                    ...
                    self.iter1 = 1
                self.iter3 += 1
                return self.TrainweightedCEL(logits, target)
        else:
            return self.ValweightedCEL(logits, target)
...
class Metrics(nn.Module):
    ...
    """
    def __init__(self, num_classes : int , id2label : dict , top_k = 1 , average = 'none' , multidim_average='global', ignore_index=None, validate_args=False) -> None:
        ...
        self.confusionMatrix = MulticlassConfusionMatrix( num_classes = self.num_classes , ignore_index=self.ignore_index , normalize='none' , validate_args=self.validate_args)
        self.multiF1 = MulticlassF1Score(self.num_classes, self.top_k, self.average, self.multidim_average, self.ignore_index, self.validate_args)
        self.multiRec = MulticlassRecall(self.num_classes, self.top_k, self.average, self.multidim_average, self.ignore_index, self.validate_args)
        self.multiPrec = MulticlassPrecision(self.num_classes, self.top_k, self.average, self.multidim_average, self.ignore_index, self.validate_args)
        self.multiAcc = MulticlassAccuracy(self.num_classes, self.top_k, self.average, self.multidim_average, self.ignore_index, self.validate_args)
        self.scalarMacroF1 = MulticlassF1Score(self.num_classes, self.top_k, 'macro', self.multidim_average, self.ignore_index, self.validate_args)
        self.scalarWeightedF1 = MulticlassF1Score(self.num_classes, self.top_k, 'weighted', self.multidim_average, self.ignore_index, self.validate_args)
        self.scalarRec = MulticlassRecall(self.num_classes, self.top_k, 'macro', self.multidim_average, self.ignore_index, self.validate_args)
        self.scalarPrec = MulticlassPrecision(self.num_classes, self.top_k, 'macro', self.multidim_average, self.ignore_index, self.validate_args)
        self.scalarAcc = MulticlassAccuracy(self.num_classes, self.top_k, 'macro', self.multidim_average, self.ignore_index, self.validate_args)
...
def load_model(model , PREFormer , optimizer , criterion , path):
    ...
    PREFormer_params = [ param for param in PREFormer.parameters() if param.requires_grad == True] if PREFormer is not None else []
    optimizer = AdamW(model_params + PREFormer_params)
